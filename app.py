    ...
    if not st.session_state['logged_in']:
        ...
        with st.form("score_form", clear_on_submit=True):
            ...
            if "Report" in f_stage:
                ...
                st.subheader("🏗️ Design Presentation")
                m_c1 = st.select_slider("Criteria 1", mark_options, 0.0)
                m_c2 = st.select_slider("Criteria 2", mark_options, 0.0)
                m_c3 = st.select_slider("Criteria 3", mark_options, 0.0)
                raw_mark = float(m_c1 + m_c2 + m_c3)
...
# --- ROLE: COORDINATOR ---
    ...
    if (project_type == "Research Project" and pwd == "Blackberry") or (project_type == "Design Project" and pwd == "Apple"):
        ...
        if not base_df.empty and not md.empty:
            # Clean marks data IDs to ensure matching
            ...
            # Merge registration info with calculated marks
